    ...
    if coffee_data_df is None:
        try:
            ...
            if os.path.exists(DATA_FILE_PATH):
                 ...
                 coffee_data_df['Roast.Level'] = coffee_data_df['Roast.Level'].fillna('Medium') # Example default
...
def find_beans_by_flavor(keywords: List[str], df: pd.DataFrame) -> Optional[pd.DataFrame]:
    ...
    # Simple search logic (can be improved significantly)
    # Combine relevant text fields for searching
    df['search_text'] = df['Species'].fillna('') + ' ' + \
                        df['Variety'].fillna('') + ' ' + \
                        df['Processing.Method'].fillna('') + ' ' + \
                        df['Flavor.Notes'].fillna('') # Assuming a column like this exists or is created
    ...
    if not results.empty:
        # Could return top N results, potentially summarizing them
